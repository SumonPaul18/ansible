...
- **এজেন্টলেস**: টার্গেট মেশিনে এজেন্ট ইনস্টল করার প্রয়োজন নেই।
- **সহজ**: YAML ভিত্তিক প্লেবুক লেখা এবং বোঝা সহজ।
...
## **Ansible এর মূল ধারণা** <a id="ansible-এর-মূল-ধারণা"></a>

1. **কন্ট্রোল নোড**: যে মেশিনে Ansible ইনস্টল করা আছে এবং যেখান থেকে কমান্ড/প্লেবুক চালানো হয়।
2. **ম্যানেজড নোড**: Ansible দ্বারা ম্যানেজ করা রিমোট সার্ভার।
3. **ইনভেন্টরি**: ম্যানেজড নোডের তালিকা সহ একটি ফাইল।
4. **মডিউল**: নির্দিষ্ট টাস্ক সম্পাদন করে যেমন—প্যাকেজ ইনস্টল, ইউজার তৈরি ইত্যাদি।
5. **প্লেবুক**: YAML ফাইলে টাস্ক ডিফাইন করা হয়।
6. **রোল**: পুনঃব্যবহারযোগ্য টাস্ক, টেমপ্লেট এবং ভ্যারিয়েবলের সেট।
7. **অ্যাড-হক কমান্ড**: দ্রুত কাজ করার জন্য এক-লাইন কমান্ড।
...
**🧠 Ansible কী?**
Ansible হলো একটি ওপেন সোর্স automation tool যেটা দিয়ে আপনি সহজেই configuration management, application deployment, task automation, network automation এবং multi-node orchestration করতে পারেন। এটি আপনাকে একটি একক কন্ট্রোল মেশিন থেকে একাধিক সার্ভার (নোড) ম্যানেজ করতে সাহায্য করে যেখানে ম্যানেজ করা নোডগুলিতে অতিরিক্ত সফটওয়্যার ইনস্টল করার প্রয়োজন হয় না।
